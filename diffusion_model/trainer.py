...
class GaussianDiffusion(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.ll_gamma_max   = float(ll_gamma_max)
    ...
    def p_losses(self, x_start, t, condition_tensors=None, noise=None):
        ...
        if self.predict_x0:
            ...
            # ---------------- LL mean / std regs
            # placeholders; will be overwritten by body-masked LL regs below
            ll_mu_loss = x_start.new_tensor(0.0)
            ll_std_loss = x_start.new_tensor(0.0)
            ll_mu_reg  = x_start.new_tensor(0.0)
            ll_std_reg = x_start.new_tensor(0.0)
            ...
            # --- Body mask at coeff resolution (for LL stats & optional HF body loss)
            # Downsample image-space body mask to LL/coeff resolution: [B,1,H/2,W/2]
            body_coeff = F.interpolate(body_mask_img, size=(h, w), mode='area')
            body_coeff = (body_coeff > 0.1).float()

            # ---------------- Body-masked, boundary-weighted LL μ / σ regs ----------------
            # Masked LL mean/std per sample: only over body, no air.
            x_ll_tgt  = x_start[:, :1]   # [B,1,H/2,W/2]
            x_ll_pred = x0_pred[:, :1]

            # common denominator: number of body pixels per sample
            denom = body_coeff.sum(dim=(2, 3), keepdim=True).clamp_min(1e-6)  # [B,1,1,1]

            # masked means
            mu_tgt = (x_ll_tgt * body_coeff).sum(dim=(2, 3), keepdim=True) / denom
            mu_pred = (x_ll_pred * body_coeff).sum(dim=(2, 3), keepdim=True) / denom

            # masked variances
            var_ll_tgt = ((x_ll_tgt - mu_tgt) ** 2 * body_coeff).sum(dim=(2, 3)) / denom.view(-1, 1)
            var_ll_pred = ((x_ll_pred - mu_pred) ** 2 * body_coeff).sum(dim=(2, 3)) / denom.view(-1, 1)

            std_ll_tgt = (var_ll_tgt + 1e-8).sqrt()
            std_ll_pred = (var_ll_pred + 1e-8).sqrt()

            # per-sample LL μ / σ errors (before boundary weighting)
            # shape [B]
            ll_mu_err  = (mu_pred - mu_tgt).pow(2).view(x_start.size(0), -1).mean(dim=1)
            ll_std_err = (torch.log(std_ll_pred) - torch.log(std_ll_tgt)).pow(2).view(x_start.size(0), -1).mean(dim=1)

            # ---------------- Boundary weight γ based on body area ----------------
            # Intuition: slices with smaller body area (legs / abdominal fringe)
            # get larger γ → stronger LL anchoring.
            body_area = body_coeff.view(x_start.size(0), -1).mean(dim=1)        # [B], fraction of body pixels
            inv_area  = 1.0 / (body_area + 1e-6)
            inv_norm  = inv_area / inv_area.mean()                              # normalize across batch
            gamma = inv_norm.clamp(min=1.0, max=self.ll_gamma_max)                            # [B], tune max=2–3 if needed

            # final LL losses (scalar)
            ll_mu_reg  = ll_mu_err.mean()        # for logging (before γ)
            ll_std_reg = ll_std_err.mean()

            ll_mu_loss = (gamma * ll_mu_err).mean()  * self.ll_mu_coef
            ll_std_loss = (gamma * ll_std_err).mean() * self.ll_std_coef
            ...
            if getattr(self, "lambda_body_hf", 0.0) > 0.0:
                # HF reconstruction error [B,3,H/2,W/2] within body
                hf_err = (x0_pred[:, 1:] - x_start[:, 1:]).abs()
                denom_hf = body_coeff.sum(dim=(2,3), keepdim=True) * 3.0 + 1e-8
                body_hf_err = (hf_err * body_coeff).sum(dim=(2,3), keepdim=True) / denom_hf
                body_hf_loss = self.lambda_body_hf * body_hf_err.mean()
            ...
            # --- LL-dependent, background-only anti-saturation in pixel space
            sat_loss = 0.0
            if self.sat_lambda > 0.0:
                # 1) Build background mask: body minus a dilated lesion core
                Himg, Wimg = img_pred.shape[-2:]
                # dilate lesion in image space to get a safety ring
                lesion_dil_img = F.max_pool2d(m_img_core, kernel_size=7, stride=1, padding=3)
                bg_mask_img = (body_mask_img * (1.0 - lesion_dil_img)).clamp(0, 1)  # body background only

                # 2) Upper-saturation penalty on normalized intensities in [0,1]
                img01 = (img_pred.clamp(-1.0, 1.0) + 1.0) * 0.5            # [B,1,H,W]
                over  = F.relu(img01 - self.sat_thresh)                     # >0 only where near top of window

                # per-sample background saturation cost: mean(over^2) over bg pixels
                bg_sum = bg_mask_img.sum(dim=(2, 3)) + 1e-8                 # [B]
                sat_bg = (over.pow(2) * bg_mask_img).sum(dim=(2, 3)) / bg_sum  # [B]

                # 3) LL-dependent gate: stronger when LL mean has drifted
                #    delta_mu: |μ_LL_pred - μ_LL_tgt| per sample (already computed above)
                delta_mu = (mu_pred - mu_tgt).abs().view(b, -1).mean(dim=1)     # [B]
                # soft gate in [0,1]; tolerate ~0.01 mean error before activating strongly
                gate = torch.sigmoid((delta_mu - 0.01) / 0.005)                 # [B]

                # 4) Combine with boundary γ: legs / abdomen (small body area) already have larger γ
                #    (gamma was computed above from body_coeff and is shape [B])
                sat_weight = (gamma * gate)                                     # [B]

                # 5) Final LL-dependent saturation loss
                sat_loss = self.sat_lambda * (sat_weight * sat_bg).mean()
