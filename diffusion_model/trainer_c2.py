...
def _ensure_ctx5d(ctx: torch.Tensor) -> torch.Tensor:
    """
    Make sure ctx is [B, C_ctx, H, W, D]. If it's [B,C,H,W], add D=1.
    """
    if ctx is None:
        return None
    if ctx.ndim == 4:
    ...
    if ctx.ndim == 5:
        return ctx
    raise ValueError(f"ctx must be 4D or 5D, got shape={tuple(ctx.shape)}")
...
class GaussianDiffusion(nn.Module):
    ...
    @torch.no_grad()
    def p_mean_variance(self, x, t, clip_denoised: bool, c=None, soft_a=None, **model_kwargs):
        """
        ...
        """
        # --- optional 3D context for UNet (C2)
        ctx5d = _ensure_ctx5d(model_kwargs.get('ctx', None))
        ...
        if self.predict_x0:
            ...
            if self.with_condition and (c is not None) and (self.cfg_mask_scale > 0 or self.cfg_neighbor_scale > 0):
                ...
                x0_null = self.denoise_fn(self._prep_model_in(x, c_null), t, ctx=ctx5d)

                c_m = torch.cat([c_mask, torch.zeros_like(c_neighbor)], dim=1)
                x0_m = self.denoise_fn(self._prep_model_in(x, c_m), t, ctx=ctx5d)

                if c_neighbor.numel() > 0:
                    c_n = torch.cat([torch.zeros_like(c_mask), c_neighbor], dim=1)
                    x0_n = self.denoise_fn(self._prep_model_in(x, c_n), t, ctx=ctx5d)
                else:
                ...
                x0_hat = self.denoise_fn(model_in, t, ctx=ctx5d)

        else:
            # ε-pred; (CFG omitted here since you use predict_x0=True)
            eps = self.denoise_fn(self._prep_model_in(x, c), t, ctx=ctx5d)
    ...
    def p_sample(self, x: torch.Tensor, t: torch.LongTensor, condition_tensors=None,
                 clip_denoised: bool = True, repeat_noise: bool = False, soft_a=None, **model_kwargs):
        ...
        model_mean, _, model_log_variance = self.p_mean_variance(
            x=x, t=t, c=c, clip_denoised=clip_denoised, soft_a=soft_a, **model_kwargs
        )
    ...
    @torch.no_grad()
    def p_sample_loop(self, shape, condition_tensors=None, clip_denoised: bool = True, soft_a=None, **model_kwargs):
        ...
        for i in tqdm(reversed(range(self.num_timesteps)), total=self.num_timesteps, desc="Sampling"):
            ...
            img = self.p_sample(img, t, condition_tensors=c, clip_denoised=clip_denoised, soft_a=soft_a,ctx=ctx5d)
    ...
    def p_losses(self, x_start, t, condition_tensors=None, noise=None, **model_kwargs):
        """
        Training loss with:
        ...
        if self.predict_x0:
            # x0-prediction path
            x0_pred = self.denoise_fn(model_in, t, ctx=ctx5d)
            ...
            eps_pred = self.denoise_fn(model_in, t, ctx=ctx5d)
            if self.loss_type == 'l2':
