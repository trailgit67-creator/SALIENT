...
class SliceItem:
    ...
    slice_id: per-subject slice identifier (usually the middle token in {sid}_{slice}_ct.png)
...
# -----------------------------------------------------------------------------
# Synthetic positives (unchanged location)
...
def index_synthetic_slices(positives_root: Path) -> List[SliceItem]:
    ...
    Synthetic generation directory structure (unchanged):
    ...
    for subj_dir in sorted(root.iterdir()):
        ...
        gen_dir = subj_dir / "positive" / "wdm_generation_p4_v1"
        mk_dir  = subj_dir / "positive" / "mask_v1"
...
#          {sid}/positive_crop/{sid}_{slice}_ct.png
#          {sid}/positive_crop/{sid}_{slice}_mask.png
#      * Negative slices (from positive subjects)
#          {sid}/negative_crop/{sid}_{slice}_ct.png
#
...
def _index_positive_crop_for_subject(subj_dir: Path) -> List[SliceItem]:
    """Index positive slices from {sid}/positive_crop."""
    items: List[SliceItem] = []
    sid = subj_dir.name
    pos_dir = subj_dir / "positive_crop"
    if not pos_dir.exists():
        return items

    # Strategy: pair *_ct.png with matching *_mask.png in the same folder.
    ...
    for p in pos_dir.iterdir():
        if not (p.is_file() and _is_png(p)):
            continue
        name = p.name
        if name.endswith("_ct.png"):
            ct_files[name.replace("_ct.png", "")] = p
        elif name.endswith("_mask.png"):
            mask_files[name.replace("_mask.png", "")] = p

    common_keys = sorted(set(ct_files.keys()) & set(mask_files.keys()))
    for key in common_keys:
        ct_path = ct_files[key]
        mk_path = mask_files[key]
        stem = ct_path.stem  # e.g., "{sid}_{slice}_ct"
        # remove sid_ prefix and trailing _ct to define slice_id
        toks = stem.split("_", 1)  # ["sid", "slice_ct"]
        rest = toks[1] if len(toks) > 1 else stem
        rest = rest.replace("_ct", "")
        slice_id = rest
...
def index_positive_slices(positives_root: Path) -> List[SliceItem]:
    ...
    We now assume positives are stored under:
      {positives_root}/{sid}/positive_crop/{sid}_{slice}_ct.png
      {positives_root}/{sid}/positive_crop/{sid}_{slice}_mask.png
    """
    items: List[SliceItem] = []
    root = Path(positives_root)
...
def index_negative_slices_from_positive_subjects(positives_root: Path) -> List[SliceItem]:
    ...
    Index NEGATIVE slices that come from positive subjects:
      {positives_root}/{sid}/negative_crop/{sid}_{slice}_ct.png
    ...
    for subj_dir in sorted(root.iterdir()):
        ...
        neg_dir = subj_dir / "negative_crop"
        ...
        for p in sorted(neg_dir.iterdir()):
            if not (p.is_file() and _is_png(p)):
                continue
            stem = p.stem  # e.g., "{sid}_{slice}_ct"
            toks = stem.split("_", 1)
            rest = toks[1] if len(toks) > 1 else stem
            rest = rest.replace("_ct", "")
            slice_id = rest
...
def build_items(
    ...
    negatives_root: str = "/home/li46460/TRAIL_Yifan/negative_control",
) -> Tuple[List[SliceItem], List[SliceItem]]:
    """
    Convenience wrapper returning:
      pos_all : all REAL positive slices from positive_crop
      neg_all : all NEGATIVE slices from
                  - negative_crop of positive subjects
