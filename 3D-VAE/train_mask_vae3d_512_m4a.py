...
# -*- coding: utf-8 -*-
"""
train_mask_vae3d_512_m4a.py — M4A (ROI-conditioned M4)

Base: M4 (nearest-neighbor decoder + boundary-band BCE + curriculum for SDF/skeleton +
       stronger edge later + post ResBlock3D + gentle KL/free-bits)
Adds: ROI conditioning (mediastinum mask) for positional anchoring.

Data expectation (per subject id folder):
  {sid}/positive/mask/*.png            -> hematoma masks (binary)
  {sid}/mediastinum_mask/*.png         -> ROI masks (binary), SAME filenames as mask

Training/inference I/O:
  Encoder sees (mask, ROI). Decoder is conditioned on ROI at each fusion stage.
  Sampling requires an ROI volume: sample(B, device, tau, roi).
...
class VolumeBlocksFromPNGs(Dataset):
    """
    Returns paired volumes: (mask_vol01, roi_vol01), both in [0,1], shape [1,D,H,W]
    Pairs mask PNGs with ROI PNGs by identical filenames under mediastinum_mask/.
    Blocks are contiguous runs of slices (split at index gaps > gap_split).
    """
    def __init__(self, data_root: str, D=64, H=256, W=256, gap_split: int = 10, min_pos_frac: float = 0.001):
        root = Path(data_root)
        subs = [d for d in root.iterdir() if d.is_dir() and _is_int(d.name)]
        self.blocks = []  # list of lists of (mask_path, roi_path)
        for sid_dir in sorted(subs, key=lambda p: int(p.name)):
            mk = sid_dir / "positive" / "mask"
            rk = sid_dir / "mediastinum_mask"
            if not mk.exists() or not rk.exists():
                continue
            # Index ROI by filename for fast lookup
            roi_by_name = {p.name: p for p in rk.glob("*.png")}
            files = sorted(mk.glob("*.png"))
            pairs = []
            for mp in files:
                if mp.name not in roi_by_name:
                    # Skip if ROI missing for this slice
                    continue
                m = SLICE_RE.search(mp.name)
                if m: sl = int(m.group(2))
                else:
                    digits = re.findall(r"(\d+)", mp.stem)
                    if not digits:
                        continue
                    sl = int(digits[-1])
                pairs.append((sl, mp, roi_by_name[mp.name]))
            if not pairs: 
                continue
            pairs.sort(key=lambda t: t[0])
            current = [(pairs[0][1], pairs[0][2])]
            for i in range(1, len(pairs)):
                prev_idx = pairs[i-1][0]
                curr_idx, mp, rp = pairs[i]
                if curr_idx - prev_idx > gap_split:
                    if len(current) > 0: self.blocks.append(current)
                    current = [(mp, rp)]
                else:
                    current.append((mp, rp))
            if len(current) > 0:
                self.blocks.append(current)
        if not self.blocks:
            raise RuntimeError(f"No paired (mask, ROI) volumes found under {data_root}")
    ...
    def __getitem__(self, idx):
        pairs = self.blocks[idx]
        mask_slices = [_read_mask_native(mp) for (mp, _) in pairs]
        roi_slices  = [_read_mask_native(rp) for (_, rp) in pairs]

        refH, refW = mask_slices[0].shape[-2], mask_slices[0].shape[-1]
        D0 = len(mask_slices)

        mvol = torch.zeros(1, D0, refH, refW, dtype=torch.float32)
        rvol = torch.zeros(1, D0, refH, refW, dtype=torch.float32)
        for i, (ms, rs) in enumerate(zip(mask_slices, roi_slices)):
            if ms.shape[-2:] != (refH, refW):
                ms = F.interpolate(ms.unsqueeze(0), size=(refH, refW), mode="nearest")[0]
            if rs.shape[-2:] != (refH, refW):
                rs = F.interpolate(rs.unsqueeze(0), size=(refH, refW), mode="nearest")[0]
            mvol[:, i] = ms
            rvol[:, i] = rs

        mvol = _resize_volume_nn(mvol, self.D, self.H, self.W)  # [1,D,H,W]
        rvol = _resize_volume_nn(rvol, self.D, self.H, self.W)  # [1,D,H,W]

        if mvol.sum() / float(self.D*self.H*self.W) < self.min_pos_frac:
...
# ------------------------------- model (M4 + ROI conditioning) -------------------------------
def add_coords_3d_like(feat: torch.Tensor):
    """Return 3 coord channels matching feat's spatial D,H,W."""
...
class Encoder3D(nn.Module):
    """
    ROI-conditioned encoder: input channels = 2 (mask, roi)
    """
    ...
    def forward(self, x2):  # [B,2,D,H,W] -> (mu, logvar)
        h = self._maybe_ckpt(self.stem, x2)
...
class Decoder3D(nn.Module):
    """
    ROI-conditioned decoder:
      At each fusion block f1..f5 we concat [coords(3) + roi_resized(1)] -> +4 channels.
      So input chans to f_k are (prev_c + 4).
    """
    def __init__(self, D=128, H=512, W=512, base=24, latent_dim=96, grad_ckpt=False):
        super().__init__()
        self.grad_ckpt = grad_ckpt
        ...
        # up path
        self.u1 = UpConv3D(c5, c5, scale=(2,2,2)); self.f1 = nn.Sequential(nn.Conv3d(c5+4, c5, 3,1,1), nn.SiLU(), ResBlock3D(c5))
        self.u2 = UpConv3D(c5, c4, scale=(2,2,2)); self.f2 = nn.Sequential(nn.Conv3d(c4+4, c4, 3,1,1), nn.SiLU(), ResBlock3D(c4))
        self.u3 = UpConv3D(c4, c3, scale=(2,2,2)); self.f3 = nn.Sequential(nn.Conv3d(c3+4, c3, 3,1,1), nn.SiLU(), ResBlock3D(c3))
        self.u4 = UpConv3D(c3, c2, scale=(2,2,2)); self.f4 = nn.Sequential(nn.Conv3d(c2+4, c2, 3,1,1), nn.SiLU(), ResBlock3D(c2))
        self.u5 = UpConv3D(c2, c1, scale=(2,2,2)); self.f5 = nn.Sequential(nn.Conv3d(c1+4, c1, 3,1,1), nn.SiLU(), ResBlock3D(c1))
        self.u_last = UpConv3D(c1, c1, scale=(1,2,2))
        self.post   = ResBlock3D(c1)
        self.head   = nn.Conv3d(c1, 1, 1)

    def _maybe_ckpt(self, fn, *xs):
        if self.grad_ckpt and any(getattr(x, "requires_grad", False) for x in xs):
            return ckpt(lambda *i: fn(*i), *xs)
        return fn(*xs)

    def _fuse(self, h, roi_full):
        # Resize ROI to h's spatial dims; add coords
        B, _, D, H, W = h.shape
        roi_r = F.interpolate(roi_full, size=(D,H,W), mode='nearest')
        z,y,x = add_coords_3d_like(h)
        return torch.cat([h, roi_r, z, y, x], dim=1)

    def forward(self, z, roi):  # roi: [B,1,D,H,W] full-res
        B = z.size(0)
        h = self.fc(z).view(B, self.enc_ch, *self.enc_sp)
        h = self._maybe_ckpt(self.ref, h)

        h = self._maybe_ckpt(self.u1, h); h = self._maybe_ckpt(self.f1, self._fuse(h, roi))
        h = self._maybe_ckpt(self.u2, h); h = self._maybe_ckpt(self.f2, self._fuse(h, roi))
        h = self._maybe_ckpt(self.u3, h); h = self._maybe_ckpt(self.f3, self._fuse(h, roi))
        h = self._maybe_ckpt(self.u4, h); h = self._maybe_ckpt(self.f4, self._fuse(h, roi))
        h = self._maybe_ckpt(self.u5, h); h = self._maybe_ckpt(self.f5, self._fuse(h, roi))
...
class MaskVAE3D(nn.Module):
    ...
    def forward(self, x01, roi01):
        """
        x01:  [B,1,D,H,W] target mask
        roi01:[B,1,D,H,W] ROI mask
        """
        mu, logvar = self.enc(torch.cat([x01, roi01], dim=1))
        z = self.reparameterize(mu, logvar)
        logits = self.dec(z, roi01)
        return logits, mu, logvar

    @torch.no_grad()
    def sample(self, B, device, tau=1.0, roi=None):
        assert roi is not None, "ROI volume required for ROI-conditioned sampling"
        z = torch.randn(B, self.latent_dim, device=device) * float(tau)
        return self.dec(z, roi)
...
# ------------------------------- curriculum helpers (same as M2/M3/M4) -------------------------------
...
def main():
    ...
    # io
    ap.add_argument('--out_ckpt', type=str, default='mask_vae3d_512_m4a.pth')
    ...
    with open(out_dir / "mask_vae3d_512_m4a_config.json", "w") as f: json.dump(vars(args), f, indent=2)
    ...
    for ep in range(1, args.epochs+1):
        ...
        for mvol, rvol in dl_tr:
            mvol = mvol.to(device, non_blocking=True)  # [B,1,D,H,W]
            rvol = rvol.to(device, non_blocking=True)  # [B,1,D,H,W]
            with torch.autocast(device_type='cuda', dtype=amp_dtype, enabled=use_amp):
                logits, mu, logvar = model(mvol, rvol)

                # boundary-band BCE (on target mask)
                with torch.no_grad():
                    band = boundary_band_3d(mvol, radius=args.band_radius)
                    w = 1.0 + args.band_lambda * band
                bce  = F.binary_cross_entropy_with_logits(logits, mvol, weight=w, reduction='mean')

                dice = dice_loss_from_logits_3d(logits, mvol)
                edge = edge_loss_from_logits_3d(logits, mvol)

                sdf  = sdf_loss_from_logits_3d(logits, mvol, K=args.sdf_K) if sdf_w_now > 0 else logits.new_tensor(0.0)
                skel = skeleton_dice_from_logits_3d(logits, mvol)          if skel_w_now > 0 else logits.new_tensor(0.0)
        ...
        with torch.no_grad(), torch.autocast(device_type='cuda', dtype=amp_dtype, enabled=use_amp):
            for mvol, rvol in dl_va:
                mvol = mvol.to(device, non_blocking=True)
                rvol = rvol.to(device, non_blocking=True)
                logits, mu, logvar = ema.ema(mvol, rvol)
        ...
        if ep == 1 or ep % 2 == 0:
            with torch.no_grad(), torch.autocast(device_type='cuda', dtype=amp_dtype, enabled=use_amp):
                try:
                    mval, rval = next(iter(dl_va))
                    mval = mval.to(device, non_blocking=True)
                    rval = rval.to(device, non_blocking=True)
                    logits, _, _ = ema.ema(mval, rval)
                    save_triptych_3d(mval, logits, out_dir / f"samples/ep_{ep:03d}_recon.png")
                    B = min(4, mval.size(0))
                    s_logits = ema.ema.sample(B=B, device=device, tau=args.sample_tau, roi=rval[:B])
                    s_probs  = torch.sigmoid(s_logits); s_bins = (s_probs>0.5).float()
                    mid = s_bins.shape[2]//2
                    panel = torch.cat([s_bins[:B,:,mid], s_probs[:B,:,mid]], dim=3)
